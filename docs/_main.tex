...
\begin{itemize}
\item
  \texttt{PolicyID} : identifiant unique du contrat
\item
  \texttt{ClaimNb} : nombre de sinistres sur la période (variable cible)
\item
  \texttt{Exposure} : durée d'exposition au risque (en années)
\item
  \texttt{Power} : puissance fiscale du véhicule (catégorielle)
\item
  \texttt{CarAge} : âge du véhicule (en années)
\item
  \texttt{DriverAge} : âge du conducteur principal
\item
